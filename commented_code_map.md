...
Builds and parses the CLI.

Supported CLI options:

- `-c`, `--config PATH` — required config file path.
- `--validate-config` — parse config and print the effective startup summary, but do not acquire the lock or run rclone commands.
- `--version` — print the package version and exit.
- `-h`, `--help` — generated by `argparse`; print usage and exit.

Why: validation mode gives a non-destructive way to catch path/schema mistakes before remote operations begin.
